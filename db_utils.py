...
fac_db = requests.get('https://drive.google.com/uc?export=download&id=1Tzz-FxmvzmDrZmRUYfdZDOGZD1LAXJFx').json() # структура групп
...
description_message = 'Этот бот поможет быть в курсе актуальной информации о расписании занятий'
start_message = 'Привет, студент👋 Для начала выбери свой факультет:'
choose_group_message = 'Теперь выбери свою специальность:'
choose_year_message = 'На каком курсе ты учишься?'
change_schedule_message = 'Произошли изменения в расписании🗓\nВот актуальная информация:'

day_names = ['MONDAY', 'TUESDAY', 'WEDNESDAY', 'THURSDAY', 'FRIDAY', 'SATURDAY']
lesson_nums = ['1', '2', '3', '4']
lessons_mock = [
	'Основы программирования', 'Информационная безопасность', 'Математический анализ',
	'Теория вероятности', 'Право',  'Математический анализ', 'Основы программирования',
	'Физическая культура', 'Психология', 'Экономика', 'Политология'
]
rooms_mock = ['308', '104', '212', '104', '101', '243', '124']
...
def getAllFaculties():
	return facdb['faculties'].keys()

def getAllGroups():
	return schedule.keys()

def getAllGroupsOfFaculty(faculty_name):
	return facdb['faculties'][faculty_name]['specialities'].keys()
...
def random_update():
	...
	for group in schedule.keys():
		updateSchedule( group,
						...
						(random.choice(lessons_mock) + " (" + random.choice(rooms_mock)+ ")"))
